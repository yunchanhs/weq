...
# 에이징(질질 포지션 정리)
AGING_MAX_MIN = 90                       # 진입 후 90분 지났는데
AGING_NO_HIGH_MIN = 45                   # 최근 45분 고점 미갱신 & ml 약화면 정리
...
def smart_sell_market(ticker, amount_coin):
    ...
    try:
        ...
        spread_bp = None
        try:
            if ob and 'orderbook_units' in ob[0]:
...
def get_dynamic_stop(entry_px=None, cur_px=None, ticker=None):
    """TP1 체결 이후엔 진입가 근처로 손절선 상향(브레이크이븐)"""
    base = FIXED_STOP_LOSS if USE_FIXED_STOP else -0.052
    if ticker and breakeven_on.get(ticker, False) and entry_px and cur_px:
        return max(base, (cur_px - entry_px)/entry_px - 0.002)  # -0.2% 버퍼
    return base
...
def record_partial_pnl(ticker, sell_qty, sell_px):
    ...
    try:
        ...
        record_trade_pnl(pnl_ratio * 0.5)  # 원하면 1.0로 상향 가능
    except Exception:
        pass
...
def try_hard_stop(ticker, coin_balance, now):
    try:
        ...
        stop_thr = get_dynamic_stop(entry_px=entry_px, cur_px=px_now, ticker=ticker)
...
def should_sell(ticker, current_price, ml_signal, t_sell, regime):
    ...
    if current_price > highest_prices.get(ticker, current_price):
    ...
    # 하드 스톱
    stop_thr = get_dynamic_stop(entry_px=entry, cur_px=current_price, ticker=ticker)
    if chg <= stop_thr:
...
def run_bt_wf(ticker, model, init_bal, regime):
    ...
    if data_bt is None or data_bt.empty:
        ...
        perf_bt = backtest_series(data_bt, model, init_bal, t_buy, t_sell, stop_thr=FIXED_STOP_LOSS)
    if data_bt is None or data_bt.empty or len(data_bt) < 200:
        perf_wf = 1.0
    else:
        cut = int(len(data_bt)*0.8)
        wf = data_bt.iloc[cut:].copy()
        perf_wf = backtest_series(wf, model, init_bal, t_buy, t_sell, stop_thr=FIXED_STOP_LOSS)
        ...
        while True:
            ...
            for t in list(targets):
                ...
                if t in entry_prices:
                    ...
                    stop_thr = get_dynamic_stop(entry_px=entry, cur_px=px, ticker=t)
