...
def run_bt_wf(ticker, model, init_bal, regime):
    ...
    if data_bt is None or data_bt.empty:
        ...
        perf_bt = backtest_series(data_bt, model, init_bal, t_buy, t_sell)
    # 워크포워드: 5분봉 최근 20%
...
def calc_atr_position_budget(remaining_krw, remaining_slots, atr_abs, px, equity, base_risk=0.006, high_conf=False):
    ...
    target = max(MIN_ORDER_KRW, min(budget_by_risk, 1.7*equal_split, budget_hardcap))
    return target
...
def should_sell(ticker, current_price, ml_signal, t_sell, regime):
    ...
    if chg >= tp_dyn:
        log.info(f"[{ticker}] 레짐 기반 강제 익절 도달(+{tp_dyn*100:.0f}%) → 익절")
        return True
    if chg >= 0.15:
    ...
    try:
        while True:
            ...
            for t in list(targets):
                ...
                # MACD/ADX OR 완화: 둘 중 하나만 True여도 '한 조건'으로 카운트
                # K-of-N은 (MACD, ADX, RSI, ATR) 4개 중 K개 충족 시 통과
                conds_map = {
                    "MACD": macd_cross,
                    "ADX":  adx_strong,
                    "RSI":  rsi_ok,
                ...
                can_new_buy = (not risk_block) and (remaining_slots > 0) and pass_model and \
                              ( (kpass and (strict_gate or lax_gate)) or allow_bear_buy )
                ...
                if t not in entry_prices and can_new_buy:
                    ...
                    if not (strict_gate or lax_gate): reasons.append("ML문턱미달")
                    if risk_block: reasons.append("리스크블록")
