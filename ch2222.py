...
def smart_sell_market(ticker, amount_coin):
    """슬리피지/유동성 위험 시 분할 매도 (스프레드 bps 기준 일관화)"""
    try:
        px = fresh_price(ticker) or get_current_price_cached(ticker)
        if px is None: return False
        sp_bp = get_spread_bp(ticker)  # bps
        total_krw = amount_coin * px
        need_split = (total_krw >= 3_000_000) or (sp_bp is not None and sp_bp > 20.0)
...
class TransBlock(nn.Module):
    def __init__(self, in_dim=6, d_model=32, heads=4, layers=1):  # heads 짝수 → 경고 제거
        super().__init__()
...
class HybridModel(nn.Module):
    def __init__(self, in_dim=6):
        ...
        self.head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),            # ← 버그fix
            nn.Linear(64, 1)            # BCEWithLogitsLoss 사용 → 시그모이드는 추론에서
        )
    def forward(self, x):
...
def get_spread_bp(ticker):
    ...
    try:
        ...
        if not top or "ask_price" not in top or "bid_price" not in top:
            return None
        ask = float(top["ask_price"]); bid = float(top["bid_price"])
        mid = (ask + bid) / 2.0
        if mid <= 0:
            return None
        spread = (ask - bid) / mid
...
def main():
    ...
    if not top_tickers:
        ...
        last_top_update = datetime.now() - timedelta(hours=6, minutes=1)
