...
# 포지션/현금
MAX_ACTIVE_POSITIONS_BASE = 2            # 기본 보유 슬롯
...
# >>> 요청대로 부분익절 값 고정 <<<
PARTIAL_TP1, TP1_RATIO = 0.08, 0.25      # +8% 도달시 25% 익절
PARTIAL_TP2, TP2_RATIO = 0.15, 0.15      # +15% 도달시 15% 추가 익절
...
# ---------- Priority Buy (리스크블록 중에도 우수 종목 진입 허용) ----------
PRIORITY_BUY_ENABLE = True
PRIORITY_BUY_SLOTS  = 1                  # 리스크블록 중에도 허용할 우선 슬롯 수
PRIORITY_BUY_ML_BONUS = 0.05             # T_buy보다 이만큼 추가 요구
HIGHCONF_RISK_MULT = 1.6                 # 고신뢰 리스크 단위 배수
PRIORITY_RISK_MULT = 2.0                 # Priority 리스크 단위 배수

# ===================== 상태 =====================
entry_prices = {}
entry_times  = {}        # 진입 시각 (부분익절 등으로 덮어쓰지 않음)
highest_prices = {}
recent_trades = {}
...
def safe_current_price(ticker, max_retries=5, base_sleep=0.5):
    for k in range(1, max_retries+1):
        try:
            ...
            log.info(f"[가격조회 실패] {ticker} 시도 {k}/{max_retries} → {e}")
        time.sleep(base_sleep * k)
    return None
...
def ensure_market_sell(ticker, amount_coin, retries=5, base_sleep=0.8):
    """강제손절/응급청산의 체결 신뢰도 향상을 위한 다회 재시도 + 최소주문 보정"""
    if amount_coin <= 0:
        return False
    for k in range(1, retries+1):
        try:
            px = safe_current_price(ticker) or 0.0
            if px <= 0:
                time.sleep(base_sleep * k)
                continue
            if amount_coin * px < MIN_ORDER_KRW:
                log.info(f"[{ticker}] 매도금액 최소주문 미달 → 보류 (amt*px={amount_coin*px:.0f} < {MIN_ORDER_KRW})")
                return False
            resp = sell_crypto_currency(ticker, amount_coin)
            if resp:
                return True
        except Exception as e:
            log.info(f"[{ticker}] 강제 매도 재시도 {k}/{retries} → {e}")
        time.sleep(base_sleep * k)
    return False
...
def get_dynamic_stop_by_regime(regime: str) -> float:
    ...
    return STOP_LOSS_THRESHOLD  # -5% 고정
...
def try_partial_take_profit(ticker, change_ratio, coin_balance, now):
    """
    - +8%에서 25%, +15%에서 15% 부분익절
    - 진입 15분 내 급등(+12%)이면 보류(러너 살리기)
    - ML 신호 강함(>=0.65)이면 부분익절 보류(더 태움)
    - 변동성(ATR) 높으면 익절 비율 0.6배 축소
    """
    did = False
    try:
        ...
        px_now = safe_current_price(ticker)
...
def try_trailing_stop(ticker, coin_balance, now, regime="neutral"):
    try:
        ...
        if drop_pct >= drop_threshold and coin_balance > 0:
            if ensure_market_sell(ticker, coin_balance):
...
def should_sell(ticker, current_price, ml_signal, t_sell, regime):
    ...
    stop_thr = get_dynamic_stop_by_regime(regime)  # -5%
    if chg <= stop_thr:
        log.info(f"[{ticker}] 🚨 하드 스톱 신호({stop_thr*100:.1f}%)")
        return True

    # 레짐 기반 강제 익절 (상승장: 길게, 하락장: 짧게)
    tp_dyn = get_dynamic_tp_by_regime(regime)
    if chg >= tp_dyn:
        log.info(f"[{ticker}] 🎯 레짐 강제익절 도달(+{tp_dyn*100:.0f}%)")
        return True

    # 이익권에서 ML 약화 + 트레일링 확인
    trail = TRAIL_DROP_BULL if regime=="bull" else TRAIL_DROP_BEAR
    if chg >= 0.15:
        if weak_ml or ml_signal < 0.60:
            log.info(f"[{ticker}] ✅ +15% & ML 약함 → 익절")
            return True
        else:
            return False
    if chg >= 0.10 and (weak_ml or ml_signal < 0.50):
        log.info(f"[{ticker}] ✅ +10% & ML 약함 → 익절")
        return True
    if peak_drop > trail and (weak_ml or ml_signal < 0.50):
        log.info(f"[{ticker}] 📉 트레일링 스탑 조건 충족 (고점대비 {peak_drop*100:.2f}%)")
        return True
        ...
        while True:
            ...
            for t in list(targets):
                ...
                if t in entry_prices:
                    # (A) 우선 포지션 관리 루틴: 부분익절 → 트레일링 → 하드스톱(-5%)
                    ...
                    # (B) 추가 전략 매도(ML 약화/강제 익절)
                    px_now = safe_current_price(t)
                    if px_now is None:
                        continue
                    entry = entry_prices[t]
                    chg = (px_now - entry)/entry
                    _, T_sell_eff = compute_ml_threshold(t, regime, use_lax=False)
                    will_sell = should_sell(t, px_now, ml, T_sell_eff, regime)
                    ...
                    if will_sell or force_liq:
                        try:
                            ...
                            bal = 0.0
                        if bal and bal > 0:
                            # 부분익절 시도(있으면 일부 줄이고 남은거 전량)
                            try_partial_take_profit(t, chg, bal, now)
                            bal = get_balance(coin)
